...
*** 컬렉션(그리고 제네릭)이 등장한 이유.
	...
	컨테이너가 필요하게 됨. --> System.Collection 등장 --> .NET 2.0
	부터는 System.Collection.Generic 등장

	Generic 은 NonGeneric 에 비해 TypeSafe 하고, 메모리효율을 좋게한
	컨테이너이다. 
*** System.Collection 네임스페이스
	최근 추세(?)는 *더이상 System.Collection 네임스페이스를 안쓴다* 는
	것이다. 대부분 *System.Collection.Generic 네임스페이스* 를 쓴다.

	System.Collection 의 모든 컬렉션은 ICollection(IEnumarable
...
*** System.Collection.Specialized 네임스페이스
