...
** 12장. 객체로 LINQ
   ...
   #+begin_src csharp
     ...
     class LinqTest
     {
         public static void Main()
         {
             // 내게 videoGames 목록중 공백을 포함한 것들을 알파벳 순으로 줘.
             IEnumerable<string> subset = from aGame in videoGames
                                          where aGame.Contains(" ")
                                          orderby aGame select aGame;
             // 반환타입이 복잡해? 그럼 var 를 써.
             var subset  = from aGame in videoGames
                           where aGame.Contains(" ")
                           orderby aGame select aGame;
     
             // for loop 에 바로 써
             foreach (var game in (from aGame in videoGames
                                   where aGame.Contains(" ")
                                   orderby aGame select aGame))
             {
             ...
             // 아래 처럼 하면 deferred 실제 intEnum 을 돌 때 query 가 처리 된데.
             IEnumerable<int> intEnum = from i in numbers where i < 10 select i;
     
             // 바로 처리하려면, 한번에 특정 자료형으로 변환해.
             // LINQ 확장메소드들이 좋아!
             List<int> intList = (from i in numbers where i < 10 select i).ToList<int>();
             int[] intArray = (from i in numbers where i < 10 select i).ToArray<int>();
             // 위처럼 하면 LINQ 결과를 반환값으로 쓸 수도 있어!
     
             // 객체내의 서브객체들에 접근할 수 있어.
             var fastCars = from c in myCars where c.Speed > 55 && c.Maker=="BMW"
                            select c;
     
             // LINQ 쿼리를 제네릭 아닌 컬렉션(즉 여러타입이 항목으로 들어간
             // 컬렉션)에 적용할 때 Enumerable.OfType<T>() 좋아.
             var fastCars = from c in myInventory.OfType<Car>()
                            where c.Speed > 55
                            select c;
         }
     
         // // LINQ 가 새로운 무명 데이터 타입을 생성하는 경우, var 는 못써
         // public static var GetLinqVar(ProductInfo[] pi)
         // {
         //  return from p in pi select new
         //      {
         //          p.Name,
         //          p.Description
         //      };
         ...
         // 위처럼 하려면, Non Generic Collection 을 사용해야 한단다.
         ...
         public static void Test()
         {
             ...
             // { Name = 제품B, Description = LKJ제품이다 }
             // { Name = 제품C, Description = jasdf제품이다 }
         }
     
         // LINQ Enumerable 의 확장메소드를 사용해 보기
         public void TestExtension()
         {
             foreach (var prod in (from p in allProducts
         }
     }
