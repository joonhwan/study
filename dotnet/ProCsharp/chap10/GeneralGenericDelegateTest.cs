...
//   /main:GeneralGenericDelegateTest /out:GeneralGenericDelegateTest.exe GeneralGenericDelegateTest.cs \
//   && GeneralGenericDelegateTest arg1 "arg2""
// End:
using System;
...
class Car {
	...
	// 1. Delegate 정의하기(Action<int,int> 라는 클래스가 만들어진다)
	// public delegate void Action<int,int>(int beforeSpeed, int afterSpeed);
	// 2. Delegate 객체 정의하기(private 필드 + 접근메소드)
	// private Action<int,int> speedChangeHandler;
	private Action<int, int> speedChangeHandler;

	public void RegisterSpeedChangeHandler(Action<int,int> handler) {
		speedChangeHandler += handler;
	}
	public void UnregisterSpeedChangeHandler(Action<int,int> handler) {
		speedChangeHandler -= handler;
	}
...
class GeneralGenericDelegateTest {
	// 4. 핸들러 만들기
	...
	public static void Main() {
		...
		tucson.Speed = 10;
		// 5. 핸들러 등록하기
		Action<int,int> handler = new Action<int,int>(LogSpeedChange);
