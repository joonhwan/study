...
사용자정의 Container ViewController 를 구현하려 한다면, 다른
...
- ViewController 를 초기화하기 위한 사용자 정의 초기화 메소드를
  만든다.
...
- VC 는 다른 VC로 부터 데이터를 받아서 화면에 표시하되 편집하지
  않으면 ? 그 어떤 데이터도 반환되지 않는다.
- VC 가 새로운 데이터를 입력받는 기능을 한다면, 사용자의 편집작업이
  끝나고 새로 생성된 데이터를 다른 VC 에게 반환해야 한다.
- VC 가 다른 VC 로 부터 데이터를 받고 사용자가 이를 편집할 수 있다면,
  편집 종료후 새로운 데이터를 다른 VC 에게 반환한다.
- VC 가 데이터를 주지도 받지도 않는다면, 얘는 Static View 를 표시하는 것.
- VC 가 데이터를 주지도 받지도 않지만, 내부 구현상으로 데이터를 다른
...
데이터가 VC 의 안팎으로 들락거린다면, 데이터 모델 클래스를 설계하여
다른 VC에게 전송될 데이터를 들고 있도록 할 수 도 있다.  예로,
/Your Second iOS App: Storyboards/ 에서는 마스터 VC 가 BirdSighting
객체를 사용해 sighting 과 관련된 데이터를 디테일 VC 에게 전송한다.
이런식으로 구현하면, VC 클래스의 메소드 시그너쳐를 바꾸지 않고도
추가적인 속성을 데이터에 더하는게 쉬워진다.

*** 설계된 VC 는 사용자에게 어떤 작업을 수행할 수 있게 하는지를 확인하자.
어떤 VC 는 CRUD 한다. 또 어떤 VC 는 컨텐츠의 내용을 여러 화면에 걸쳐
조회할 수 있게 한다. 그리고, 어떤 VC 는 사용자가 수행할 수 있는
작업을 제공하기도 한다. 예로, MFMailComposeViewController 는 사용자로
하여금 email 을 작성/전송할 수 있게 해 준다. 저수준의 메일전송과
관련한 상세구현을 포함하고 있는 것이다.

*** VC 가 화면에 어떻게 표시되는지 확인하자.
