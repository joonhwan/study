...
QVector<QRgb> _colorTableForMonoImage;
struct __ColorTableForMonoImage_Initializer
{
	__ColorTableForMonoImage_Initializer()
	{
		if (_colorTableForMonoImage.empty()) {
			for (int i = 0; i <= 0xFF; ++i) {
				_colorTableForMonoImage << QColor(i,i,i).rgb();
			}
		}
	}
} __colorTableForMonoImage_Initializer;
...
template<>
QPixmap
WImageBufferT<uchar,3>::toPixmap() const
{
	const cv::Mat& mat = this->matrix();
	QImage tmpImage(mat.data,
					mat.cols, mat.rows,
					mat.step,
...
template<>
QPixmap
WImageBufferT<uchar,1>::toPixmap() const
{
	const cv::Mat& mat = this->matrix();
	QImage tmpImage(mat.data,
					mat.cols, mat.rows,
	...
	tmpImage.setColorTable(_colorTableForMonoImage);
	QPixmap pixmap;
	pixmap.convertFromImage(tmpImage);
	return pixmap;
}
